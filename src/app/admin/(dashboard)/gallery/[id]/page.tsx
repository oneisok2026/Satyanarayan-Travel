...
import { GALLERY_FIELDS } from '@/components/admin/catalogue-fields';
...
export default async function EditGalleryItemPage({
  ...
  try {
    item = await getCatalogueItem('gallery', parsed.data);
  } catch (error) {
    if (isAppError(error) && error.code === 'NOT_FOUND') notFound();
    throw error;
  }

  const title = String(item.caption || item.album || 'Untitled');

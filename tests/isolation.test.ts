...
describe('no schema accepts a client-supplied owner', () => {
  ...
  it('review schema has no userId or status field', () => {
    const parsed = createReviewSchema.parse({
      packageId: 'a'.repeat(24),
      rating: 5,
      comment: 'An excellent trip with very helpful guides throughout.',
    ...
    expect(parsed).not.toHaveProperty('userId');
    expect(parsed).not.toHaveProperty('verifiedBooking');
  });
...
describe('review submission cannot be self-approved', () => {
  it('requires a substantive comment', () => {
  ...
  it('constrains the rating to 1-5', () => {
    const base = {
      packageId: 'a'.repeat(24),
      comment: 'A detailed and genuine review of the whole experience.',
    };
    ...
    expect(createReviewSchema.safeParse({ ...base, rating: 5 }).success).toBe(true);
  });

  it('rejects a future travel date', () => {
    const result = createReviewSchema.safeParse({
      packageId: 'a'.repeat(24),
      rating: 5,
      comment: 'A detailed and genuine review of the whole experience.',
